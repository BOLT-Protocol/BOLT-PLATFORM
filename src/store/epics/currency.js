...
const updateBySymbolEpic = (action$) =>
    action$.pipe(
        ...
        mergeMap((action) =>
            from(getSymbol(action.payload)).pipe(
                map(({ data }) =>
                    actions.updateListSuccess({
                        ...data,
...
const getBalanceEpic = (action$) =>
    action$.pipe(
        ...
        mergeMap((action) =>
            from(getUserBalance(action.payload)).pipe(
...
export default [currrncyListEpic, updateBySymbolEpic, getBalanceEpic];
